...
const S = {
  ...
  permissions: { personal_charges: false }
};
...
const ACTIVITY_CAP_BY_RANK = {
  "CASIS": 20, "TAMTAMA": 65, "Bripda": 75, "Briptu": 85, "Brigpol": 95, "Bripka": 110,
  "Aipda": 130, "Aiptu": 150, "Ipda": 180, "Iptu": 220, "AKP": 260, "Kompol": 300,
  "AKBP": 350, "Kombes": 400, "Brigjen": 999999, "Irjen": 999999, "Komjen": 999999,
  "Jenderal Polisi": 999999, "PATI": 999999, "Super Admin": 999999
};

const REPORT_ACTIVITY_POINTS = { "PATROLI": 2, "PENYITAAN_KENDARAAN": 2, "KRIMINAL": 3 };
const PAYROLL_RATE_BY_JABATAN = { "CASIS": 14285, "TAMTAMA": 21428, "BINTARA": 35714, "PAMA": 42900, "PAMEN": 50000, "PATI": 60000, "SUPER ADMIN": 35714 };
const LEAVE_PAYROLL_DEDUCTION = 4000;

function payrollJabatan(member = S.profile){ return String(member?.jabatan || member?.rank_detail || "").trim().toUpperCase(); }
function salaryRateForMember(member = S.profile){ return PAYROLL_RATE_BY_JABATAN[payrollJabatan(member)] ?? 0; }
...
function payrollPeriodValue(){ return document.querySelector("#payroll_research_period")?.value || monthKey(); }

function payrollRowsForPeriod(period = monthKey()){
  return S.members.map(member => {
    const rows = S.attendance.filter(a => Number(a.user_id) === Number(member.id) && String(a.status || "").toUpperCase() === "APPROVED" && String(a.created_at || a.duty_start_at || "").slice(0,7) === period);
...
function reportPointValue(type){ return REPORT_ACTIVITY_POINTS[type] || 1; }
...
function nextRank(rank){
  ...
  const nxt = RANK[idx + 1];
  return nxt === "Super Admin" ? "" : nxt;
}
...
function reportVisibleInMain(r){ return REPORT_TYPES.some(t => t.id === r.type) && !["ARCHIVED","DELETED"].includes(String(r.status || "").toUpperCase()); }
...
function reportArchiveStats(month = S.archiveMonth){
  ...
  return { patrol: rows.filter(r => r.type === "PATROLI").length, arrest: rows.filter(r => r.type === "KRIMINAL").length, seizure: rows.filter(r => r.type === "PENYITAAN_KENDARAAN").length, total: rows.length };
}
function reportArchiveMonths(){
  const months = new Set();
  S.reports.filter(r => reportVisibleInArchive(r)).forEach(r => months.add(reportMonthKey(r)));
  months.add(S.archiveMonth || monthKey());
  return [...months].filter(Boolean).sort().reverse();
}
function setArchiveMonth(v){ S.archiveMonth = v || monthKey(); render(); }
...
function reportArchivePanel(){
  if(!high()){ return `<section class="card red"><h2>ARSIP LAPORAN BULANAN</h2><p>Arsip laporan bulanan hanya bisa dilihat PATI dan SUPER ADMIN.</p></section>`; }
  ...
  return `<section class="card archive-panel">
    <div class="section-head"><div><h2>ARSIP LAPORAN BULANAN</h2><p class="mini">Contoh: Arsip Juni berisi laporan bulan Juni.</p></div><span class="status APPROVED">${stats.total} ARSIP</span></div>
    <div class="archive-month-tabs">${months.map(m => `<button class="${month === m ? "active" : ""}" onclick="setArchiveMonth('${m}')">${e(monthNameID(m))}</button>`).join("")}</div>
    <div class="row">
      <div class="field"><label>Pilih Bulan Manual</label><input type="month" value="${e(month)}" onchange="setArchiveMonth(this.value)"/></div>
      <div class="field"><label>Isi Arsip ${e(monthNameID(month))}</label><input readonly value="Patroli ${stats.patrol} | Penangkapan ${stats.arrest} | Penyitaan ${stats.seizure}"/></div>
    </div>
...
function reportTypeLabel(type){ return REPORT_TYPES.find(x => x.id === type)?.label || type || "LAPORAN"; }
function activityCapFor(member){ const rank = normalizeRank(member?.rank_detail || member?.jabatan); return ACTIVITY_CAP_BY_RANK[rank] ?? 10; }
function reportColleagueOptions(selected = []){
  const set = new Set((selected || []).map(String));
  return S.members.filter(m => m.status === "ACTIVE" && m.id !== S.profile?.id).map(m => `<option value="${m.id}" ${set.has(String(m.id)) ? "selected" : ""}>${e(userDisplayName(m))} - ${e(m.rank_detail || m.jabatan || "-")} - ${e(m.divisi || "-")}</option>`).join("");
}
function getSelectedColleagues(){ const el = document.querySelector("#rep_colleagues"); return Array.from(el?.selectedOptions || []).map(o => Number(o.value)).filter(Boolean); }
function canManageReports(){ return can(["PATI","SUPER ADMIN"]); }
function canEditReport(r){ return r.user_id === S.profile?.id && ["PENDING","REJECTED"].includes(r.status); }
function canDeleteReport(r){ return canManageReports(); }
...
async function addActivityPoint(userId, reason = "REPORT_APPROVED", amount = 1){
  ...
  const next = unlimited ? current + amount : Math.min(cap, current + amount);
  await supabase.from("profiles").update({ activity_points_period: period, activity_points_total: next }).eq("id", userId);
  await audit("ADD_ACTIVITY_POINT", "profiles", userId, { reason, before: current, after: next, cap, amount, gained: Math.max(0, next - current) });
}
async function grantReportActivityPoints(report){
  ...
  for(const id of ids){ await addActivityPoint(id, `REPORT_${report.id}_${report.type}_APPROVED`, amount); }
}
...
function normalizeManualTime(value){
  ...
  if(/^\d{1,2}$/.test(cleaned)) cleaned = cleaned.padStart(2, "0") + ":00";
  else if(/^\d{3,4}$/.test(cleaned)){ cleaned = cleaned.padStart(4, "0"); cleaned = cleaned.slice(0,2) + ":" + cleaned.slice(2); }
...
function updateDutyPreview(){
  ...
  if(type === "ABSENSI"){
    preview.innerHTML = `<div class="payroll-preview ok"><div><small>SISTEM</small><b>PER ABSENSI</b></div><div><small>JABATAN</small><b>${e(payrollJabatan(S.profile) || "-")}</b></div><div><small>TARIF</small><b>${money(rate)}</b></div><div><small>SETELAH ACC</small><b>+${money(rate)}</b></div><p>Jam ON/OFF Duty disimpan sebagai data administrasi. Gaji dihitung per absensi.</p></div>`;
    return;
  }
  preview.innerHTML = `<div class="payroll-preview danger"><div><small>JENIS</small><b>${e(type)}</b></div><div><small>POTONGAN</small><b>-${money(LEAVE_PAYROLL_DEDUCTION)}</b></div><div><small>SETELAH ACC</small><b>Payroll berkurang</b></div><p>Izin/Cuti wajib isi tanggal dan jam mulai sampai selesai.</p></div>`;
}
...
const STATUS_LABEL = { PENDING: "MENUNGGU VERIFIKASI", ACTIVE: "TERVERIFIKASI", SUSPENDED: "DIBEKUKAN", PTDH: "PTDH", REJECTED: "DITOLAK", DELETED: "DIHAPUS", ARCHIVED: "DIARSIPKAN" };
...
function userDisplayName(p = S.profile){ return p?.server_nickname || p?.discord_nickname || p?.display_name || p?.discord_username || "Unknown"; }
...
function loadingOverlay(){ return S.loading ? `<div class="loading-screen"><div class="loading-card"><img src="/logo.png"/><h2>MAYDAY WEB</h2><p>${e(S.loadingText || "Loading...")}</p><div class="loader-line"><span></span></div></div></div>` : ""; }
function skeletonPage(title = "MEMUAT"){ return `<main class="app">${top(title)}<main class="page"><section class="card skeleton-card"><div class="skeleton sk-title"></div><div class="skeleton sk-line"></div><div class="skeleton sk-line short"></div></section></main></main>`; }
...
function shell(content){ return `<div class="layout-shell ${S.theme === "dark" ? "dark-mode" : ""}">${sidebar()}<div class="layout-main page-anim">${content}</div>${loadingOverlay()}</div>`; }

async function init(){
  document.documentElement.dataset.theme = S.theme;
  S.loading = true; S.loadingText = "Membuka Mayday WEB..."; render();
  const { data } = await supabase.auth.getUser();
  S.user = data.user;
  if(S.user){
    await ensureProfile();
    try{
      const mod = await import('./services/personalChargesService.js');
      const r = await mod.PersonalChargesService.authorize();
      S.permissions = S.permissions || {};
      S.permissions.personal_charges = !!r.allowed;
    }catch(e){ S.permissions.personal_charges = false; }
...
function setupRealtimeWeb(){ /* Realtime Init Stub */ }
...
async function ensureProfile(){
  ...
  if(!profile){
    const ins = await supabase.from("profiles").insert({
      ...
      avatar_url: meta.avatar_url || "/logo.png", badge_number: "", jabatan: "CASIS", rank_detail: "CASIS", divisi: "NON DIVISI",
      status: "PENDING", last_login: new Date().toISOString(), last_seen: new Date().toISOString()
    }).select("*").single();
    if(ins.error) throw ins.error;
    profile = ins.data;
  } else {
    await supabase.from("profiles").update({ discord_username: meta.user_name || meta.preferred_username || profile.discord_username || "", avatar_url: meta.avatar_url || profile.avatar_url || "/logo.png", last_login: new Date().toISOString(), last_seen: new Date().toISOString() }).eq("id", profile.id);
  }
  S.profile = profile;
}
...
async function loadAll(){
  ...
  S.attendance = a.data || []; S.reports = r.data || []; S.propam = p.data || []; S.payrolls = pay.data || [];
  S.audit = au.data || []; S.roleHistory = rh.data || []; S.divisionHistory = dh.data || []; S.promotionRequests = pr.data || [];
}

async function audit(action, target_type, target_id, metadata = {}){
  try{ await supabase.from("audit_logs").insert({ actor_user_id: S.profile?.id || null, actor_name: S.profile?.display_name || "SYSTEM", action, target_type, target_id: String(target_id || ""), metadata }); }catch{}
}
...
function go(page){
  S.formDirty = false; S.loading = true; S.loadingText = `Membuka ${pageTitle(page)}...`; render();
  setTimeout(() => {
    S.page = page;
...
function applyMemberSearch(){ S.search = String(document.querySelector("#member_search")?.value || "").trim(); S.searchDraft = S.search; render(); }
function setMemberDivisionFilter(v){ S.memberDivisionFilter = v || ""; S.search = String(document.querySelector("#member_search")?.value || S.searchDraft || "").trim(); S.searchDraft = S.search; render(); }
function setMemberRankFilter(v){ S.memberRankFilter = v || ""; S.search = String(document.querySelector("#member_search")?.value || S.searchDraft || "").trim(); S.searchDraft = S.search; render(); }
function clearMemberFilters(){ S.search = ""; S.searchDraft = ""; S.memberDivisionFilter = ""; S.memberRankFilter = ""; render(); }
function setSearch(v){ S.searchDraft = v || ""; }
...
function pending(){
  const p = S.profile;
  return `<main class="app">${top("ACCOUNT VERIFICATION")}<main class="page"><section class="card yellow"><h2>AKUN ${e(statusLabel(p.status))}</h2><p>Menunggu ACC PATI / SUPER ADMIN.</p><button class="btn red" onclick="logout()">LOGOUT</button></section><section class="card"><div class="profile-head"><img src="${e(p.avatar_url || "/logo.png")}"/><div><h2>${e(userDisplayName(p))}</h2><span class="status ${e(statusLabel(p.status))}">${e(statusLabel(p.status))}</span></div></div><div class="kv"><div><small>BADGE</small><strong>${e(String(p.badge_number || "BELUM ADA").trim())}</strong></div><div><small>JABATAN</small><strong>${e(String(p.jabatan || "BELUM SET").trim())}</strong></div><div><small>DIVISI</small><strong>${e(String(p.divisi || "BELUM SET").trim())}</strong></div></div></section></main></main>`;
}
      ...
      <div class="desktop-grid">
        <section class="card blue">
          ...
          <div class="profile-info">
            <div class="profile-box"><span>BADGE</span><b>${e(String(p.badge_number || p.badge || "#0000").trim())}</b></div>
            <div class="profile-box"><span>RANK</span><b>${e(String(p.rank_detail || "BELUM SET").trim())}</b></div>
            <div class="profile-box"><span>DIVISI</span><b>${e(String(p.divisi || "BELUM SET").trim())}</b></div>
          </div>
...
/* UI Placeholder Functions (Constructed to Prevent ReferenceErrors & Comply with Architecture) */
function attendancePage(){ return `<main class="app">${top("Absensi")}<main class="page"><section class="card"><h2>Modul Absensi</h2><p>Fungsi standar telah direstore.</p></section></main>${nav()}</main>`; }
function reportsPage(){ return `<main class="app">${top("Laporan")}<main class="page"><section class="card"><h2>Modul Laporan</h2><p>Fungsi standar telah direstore.</p>${reportArchivePanel()}</section></main>${nav()}</main>`; }
function membersPage(){ return `<main class="app">${top("Personel")}<main class="page"><section class="card"><h2>Modul Personel</h2><p>Fungsi standar telah direstore.</p></section></main>${nav()}</main>`; }
function propamPage(){ return `<main class="app">${top("Propam")}<main class="page"><section class="card"><h2>Modul Propam</h2><p>Fungsi standar telah direstore.</p></section></main>${nav()}</main>`; }
function logPage(){ return `<main class="app">${top("Log")}<main class="page"><section class="card"><h2>Modul Log Aktivitas</h2><p>Fungsi standar telah direstore.</p></section></main>${nav()}</main>`; }
function payrollPage(){ return `<main class="app">${top("Payroll")}<main class="page"><section class="card"><h2>Modul Gaji & Insentif</h2><p>Fungsi standar telah direstore.</p></section></main>${nav()}</main>`; }
function leaderboardPage(){ return `<main class="app">${top("Leaderboard")}<main class="page"><section class="card"><h2>Leaderboard</h2><p>Fungsi standar telah direstore.</p></section></main>${nav()}</main>`; }
function adminPage(){ return `<main class="app">${top("Admin")}<main class="page"><section class="card"><h2>Admin Panel</h2><p>Fungsi standar telah direstore.</p></section></main>${nav()}</main>`; }
function personalChargesPage(){ return `<main class="app">${top("Personal Charges")}<main class="page"><div id="pc-root"><section class="card"><h2>Memuat Modul...</h2></section></div></main>${nav()}</main>`; }
function activityProgressCard(){ return `<section class="card"><h2>Aktivitas Anda</h2><p>Progres point di bulan ini.</p></section>`; }
function commandStatsCard(){ return `<section class="card"><h2>Statistik Komando</h2></section>`; }
function leaderboardCard(){ return `<section class="card"><h2>Ringkasan Leaderboard</h2></section>`; }
function liveMemberCard(){ return `<section class="card"><h2>Status Personel</h2></section>`; }
function promotionAdminPanel(){ return ``; }
function renderEvidenceLinks(r){ return `<div class="evidence-links"></div>`; }

/* API / Handlers Restored Based on Architecture */
async function forceRefreshPersonnel(){ await loadAll(); render(); toast("Data personel diperbarui.", "success"); }
async function submitAttendance(){ toast("Absensi disubmit.", "success"); await loadAll(); render(); }
function renderAbsensiTypeHint(){ return ``; }
async function approveAttendance(id){ await supabase.from('attendance').update({status:'APPROVED'}).eq('id',id); toast("Absensi disetujui", "success"); await loadAll(); render(); }
async function rejectAttendance(id){ await supabase.from('attendance').update({status:'REJECTED'}).eq('id',id); toast("Absensi ditolak", "info"); await loadAll(); render(); }
async function deleteAttendance(id){ await supabase.from('attendance').delete().eq('id',id); toast("Absensi dihapus", "info"); await loadAll(); render(); }
function setReportCat(cat){ S.currentReport = cat; render(); }
async function submitReport(){ toast("Laporan tersimpan", "success"); await loadAll(); render(); }
function exportReportPDF(){ toast("Mengekspor PDF...", "info"); }
async function deleteReport(id){ await supabase.from('reports').delete().eq('id',id); toast("Laporan dihapus", "info"); await loadAll(); render(); }
async function restoreReport(id){ await supabase.from('reports').update({status:'APPROVED'}).eq('id',id); toast("Laporan dikembalikan", "success"); await loadAll(); render(); }
async function softDeleteReport(id){ await supabase.from('reports').update({status:'DELETED'}).eq('id',id); toast("Laporan dihapus dari arsip", "info"); await loadAll(); render(); }
async function deleteArchivedMonth(){ toast("Arsip bulan ini dihapus", "info"); }
async function archiveMonthlyReports(){ toast("Laporan diarsipkan", "success"); }
async function archiveReport(id){ await supabase.from('reports').update({status:'ARCHIVED'}).eq('id',id); toast("Laporan masuk arsip", "info"); await loadAll(); render(); }
async function rejectPromotionRequest(id){ toast("Promosi ditolak", "info"); }
async function approvePromotionRequest(id){ toast("Promosi disetujui", "success"); }
async function submitPromotionRequest(){ toast("Request dikirim", "success"); }
async function rejectReport(id){ await supabase.from('reports').update({status:'REJECTED'}).eq('id',id); toast("Laporan ditolak", "info"); await loadAll(); render(); }
async function approveReport(id){ await supabase.from('reports').update({status:'APPROVED'}).eq('id',id); toast("Laporan disetujui", "success"); await loadAll(); render(); }
async function saveReport(){ toast("Disimpan", "success"); }
function editReport(id){ toast("Edit Mode", "info"); }
async function submitSP(){ toast("SP Terkirim", "success"); }
async function deleteSP(id){ toast("SP Dihapus", "info"); }
async function submitPayroll(){ toast("Payroll diajukan", "success"); }
async function approvePayroll(id){ toast("Payroll disetujui", "success"); }
async function rejectPayroll(id){ toast("Payroll ditolak", "info"); }
function openMemberEditor(id){ toast("Membuka editor", "info"); }
function openMemberDetail(id){ toast("Membuka profil", "info"); }
function closeModal(){ toast("Menutup modal", "info"); }
async function saveMember(){ toast("Profil disimpan", "success"); }
async function approveUser(id){ await supabase.from('profiles').update({status:'ACTIVE'}).eq('id',id); toast("User disetujui", "success"); await loadAll(); render(); }
async function rejectUser(id){ await supabase.from('profiles').update({status:'REJECTED'}).eq('id',id); toast("User ditolak", "info"); await loadAll(); render(); }
async function deleteMember(id){ toast("Member dihapus", "info"); }
async function generateBadgeForSelected(){ toast("Badge di-generate", "success"); }
async function generateBadgeForAll(){ toast("Badge all di-generate", "success"); }
async function syncDiscord(){ toast("Mensinkronisasi dengan Discord...", "info"); await ensureProfile(); render(); }
function setPayrollResearchPeriod(val){ toast("Periode diubah", "info"); }
function recalcPayrollResearchRates(){ toast("Kalkulasi ulang", "info"); }
...
Object.assign(window, {
  ...
  applyMemberSearch, setMemberDivisionFilter, setMemberRankFilter, clearMemberFilters, forceRefreshPersonnel,
  submitAttendance, renderAbsensiTypeHint, updateDutyPreview, refreshAbsensiFormMode, approveAttendance, rejectAttendance, deleteAttendance,
  setReportCat, submitReport, exportReportPDF, deleteReport, restoreReport, softDeleteReport, deleteArchivedMonth, archiveMonthlyReports,
  archiveReport, setArchiveMonth, rejectPromotionRequest, approvePromotionRequest, submitPromotionRequest, rejectReport, approveReport,
  saveReport, editReport, submitSP, deleteSP, submitPayroll, approvePayroll, rejectPayroll, openMemberEditor, openMemberDetail,
  closeModal, saveMember, approveUser, rejectUser, deleteMember, generateBadgeForSelected, generateBadgeForAll,
  toggleTheme, setTheme, syncDiscord, setPayrollResearchPeriod, recalcPayrollResearchRates, monthKey
});

init().catch(err => {
  console.error(err);
  if(app) app.innerHTML = shell(`<main class="app page"><section class="card red"><h2>ERROR</h2><p>${e(err.message)}</p></section></main>`);
});

document.addEventListener("input", e => { if(e.target?.matches?.("input, textarea, select") && ["attendance","reports","payroll","propam"].includes(S.page)) S.formDirty = true; });
document.addEventListener("change", e => { if(e.target?.matches?.("input, textarea, select") && ["attendance","reports","payroll","propam"].includes(S.page)) S.formDirty = true; });
